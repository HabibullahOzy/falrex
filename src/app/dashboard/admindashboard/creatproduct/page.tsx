...
import {
  Upload, X, Plus, ChevronDown, Loader2,
  AlertCircle, RefreshCw, Check, Globe2,
...
// Types
// ─────────────────────────────────────────────────────────────────────────────
interface SubSubItem  { _id: string; name: string; slug: string; isActive: boolean; }
interface SubCategory { _id: string; name: string; slug: string; isActive: boolean; subSubItems: SubSubItem[]; }
interface Category    {
  _id: string; name: string; slug: string; description: string;
  image?: { url: string; public_id: string };
  subCategories: SubCategory[];
  order: number; isActive: boolean; status: string;
}

// ─────────────────────────────────────────────────────────────────────────────
// Spec definitions keyed by category SLUG
// ─────────────────────────────────────────────────────────────────────────────
type SpecDef = { key: string; label: string; fields: { key: string; label: string; placeholder: string }[] }[];

const SPEC_BY_SLUG: Record<string, SpecDef> = {
  "consumer-electronics": [
    ...
    { key: "build", label: "Build & Design", fields: [
      ...
      { key: "atmosphereLight",     label: "Atmosphere Light",    placeholder: "e.g. RGB LED" },
  ...
  "toys-hobbies": [
    ...
    { key: "safety", label: "Safety", fields: [
      ...
      { key: "materialSafety", label: "Material Safety", placeholder: "e.g. Non-Toxic Paint" },
    ]},
    { key: "business", label: "Business / OEM", fields: [
...
// Helpers
// ─────────────────────────────────────────────────────────────────────────────
function buildEmptySpecs(cats: SpecDef) {
  return Object.fromEntries(
    cats.map((c) => [c.key, Object.fromEntries(c.fields.map((f) => [f.key, ""]))])
  );
}
...
const inputCls  = "w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-400 focus:border-green-400 outline-none bg-white text-sm transition";
const selectCls = `${inputCls} appearance-none cursor-pointer`;

function SelectField({ value, onChange, disabled, placeholder, children }: {
  ...
  return (
    <div className="relative">
      <select
        ...
        disabled={disabled}
        className={`${selectCls} pr-9 ${disabled ? "opacity-50 cursor-not-allowed bg-gray-50" : ""}`}
      >
...
export default function CreateProduct() {
  ...
  const [selSubSub, setSelSubSub] = useState<SubSubItem | null>(null);
  const [hsCode,    setHsCode]    = useState("");

  // ── Specs ─────────────────────────────────────────────────────────────────
  const [specDefs,  setSpecDefs]  = useState<SpecDef>(DEFAULT_SPECS);
  ...
  const fetchCategories = async () => {
    ...
    try {
      const res  = await fetch(`${API}/category?status=approved&active=true&limit=100`);
      const json = await res.json();
      if (!json.success) throw new Error(json.message || "Failed to load categories");
      setCategories(json.data);
    } catch (err: any) {
      setCatError(err.message || "Could not load categories");
    } finally {
      setCatLoading(false);
    }
  };
  ...
  useEffect(() => {
    if (!selCat) { setSpecDefs(DEFAULT_SPECS); setSpecs(buildEmptySpecs(DEFAULT_SPECS)); return; }
    const defs = SPEC_BY_SLUG[selCat.slug] ?? DEFAULT_SPECS;
    setSpecDefs(defs);
    setSpecs(buildEmptySpecs(defs));
    setOpenSpec(null);
  }, [selCat]);

  // ── Cascade helpers ────────────────────────────────────────────────────────
  const subList    = selCat?.subCategories?.filter((s) => s.isActive) ?? [];
  const subSubList = selSub?.subSubItems?.filter((s) => s.isActive)   ?? [];

  const updateSpec = (cat: string, field: string, value: string) =>
    setSpecs((prev) => ({ ...prev, [cat]: { ...prev[cat], [field]: value } }));

  const specFilledCount = (key: string) =>
    Object.values(specs[key] ?? {}).filter(Boolean).length;

  const totalSpecFilled = specDefs.reduce((a, c) => a + specFilledCount(c.key), 0);
  const totalSpecFields = specDefs.reduce((a, c) => a + c.fields.length, 0);
  ...
  // ── Variation handlers ─────────────────────────────────────────────────────
  const addVariation    = () => setVariations((p) => [...p, { color: "", size: "", sku: "", stock: "" }]);
  const removeVariation = (i: number) => setVariations((p) => p.filter((_, idx) => idx !== i));
  const updateVariation = (i: number, field: string, value: string) => {
    setVariations((p) => { const c = [...p]; c[i] = { ...c[i], [field]: value }; return c; });
  };

  // ── Submit ─────────────────────────────────────────────────────────────────
  const onSubmit = async (data: Record<string, string>) => {
    if (!selCat) { setSubmitMsg({ type: "error", text: "Please select a category." }); return; }
    ...
    try {
      ...
      fd.set("categoryId",      selCat._id);
      fd.set("category",        selCat.name);
      fd.set("categorySlug",    selCat.slug);
      fd.set("subCategoryId",   selSub?._id    ?? "");
      fd.set("subcategory",     selSub?.name   ?? "");
      fd.set("subSubcategory",  selSubSub?.name ?? "");
      ...
      setSubmitMsg({ type: "success", text: "Product published successfully! 🎉" });
    } catch (err: any) {
      setSubmitMsg({ type: "error", text: err.message || "Failed to submit. Please try again." });
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="text-center py-6">
          <div className="inline-flex items-center gap-2 bg-green-50 border border-green-200 text-green-700 text-xs font-semibold px-3 py-1 rounded-full mb-3">
            <Globe2 className="w-3.5 h-3.5" /> B2B / OEM / ODM International Listing
          </div>
          <h1 className="text-3xl font-bold text-gray-900">Add International Product</h1>
          <p className="text-sm text-gray-500 mt-1.5">Full specification product listing for global buyers</p>
        </div>

        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">

          {/* ══ 1. Basic Information ══════════════════════════════════════ */}
          <SectionCard title="1. Basic Information" icon={<FileText className="w-4 h-4" />}>
          ...
          {/* ══ 2. Category & Classification (DYNAMIC FROM SERVER) ════════ */}
          <SectionCard title="2. Category & Classification" icon={<Tag className="w-4 h-4" />}>
            ...
            {!catLoading && catError && (
              <div className="flex items-center gap-3 bg-red-50 border border-red-200 text-red-600 rounded-xl px-4 py-3 text-sm">
                ...
                <button type="button" onClick={fetchCategories}
                  className="ml-auto flex items-center gap-1 text-xs font-semibold hover:text-red-800">
              ...
              <>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <Field label="Category" required>
                    <SelectField
                      ...
                      onChange={(id) => {
                        ...
                        setSelCat(cat); setSelSub(null); setSelSubSub(null);
                      }}
                    >
                      {categories.map((c) => (
                    ...
                    <p className="text-[11px] text-gray-400 mt-1">{categories.length} categories loaded</p>
                  </Field>
                  ...
                  <Field label="Subcategory">
                    <SelectField
                      value={selSub?._id ?? ""}
                      placeholder={selCat ? "— Select Subcategory —" : "— Pick category first —"}
                      disabled={!selCat || subList.length === 0}
                      onChange={(id) => {
                        const sub = subList.find((s) => s._id === id) ?? null;
                        setSelSub(sub); setSelSubSub(null);
                      }}
                    >
                      {subList.map((s) => (
                    ...
                    {selCat && subList.length === 0 && (
                      <p className="text-[11px] text-gray-400 mt-1">No subcategories available</p>
                    )}
                  </Field>
                  ...
                  <Field label="Sub-subcategory">
                    <SelectField
                      value={selSubSub?._id ?? ""}
                      placeholder={selSub ? "— Select Type —" : "— Pick subcategory first —"}
                      disabled={!selSub || subSubList.length === 0}
                      onChange={(id) => {
                        const ss = subSubList.find((s) => s._id === id) ?? null;
                        setSelSubSub(ss);
                      }}
                    >
                      {subSubList.map((s) => (
                    ...
                    {selSub && subSubList.length === 0 && (
                      <p className="text-[11px] text-gray-400 mt-1">No types available</p>
                    )}
                  </Field>
                </div>
                ...
                {selCat && (
                  <div className="flex items-center gap-3 flex-wrap p-3 bg-gray-50 rounded-xl border border-gray-100">
                    ...
                    <span className={`text-[10px] px-2 py-0.5 rounded-full font-semibold border flex-shrink-0 ${
                      SPEC_BY_SLUG[selCat.slug]
                        ? "bg-green-50 text-green-600 border-green-200"
                        : "bg-amber-50 text-amber-600 border-amber-200"
                    }`}>
                      {SPEC_BY_SLUG[selCat.slug] ? "✓ Specs loaded" : "⚠ Default specs"}
                    </span>
                  </div>
                )}
          ...
          {/* ══ 3. Pricing ═══════════════════════════════════════════════ */}
          <SectionCard title="3. Pricing & Order Info" icon={<Package className="w-4 h-4" />}>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <Field label="Currency">
                <div className="relative">
                  <select {...register("currency")} className={`${selectCls} pr-9`}>
              ...
              <Field label="Sample Available?">
                <div className="relative">
                  <select {...register("sampleAvailable")} className={`${selectCls} pr-9`}>
          ...
          {/* ══ 4. Supplier ══════════════════════════════════════════════ */}
          <SectionCard title="4. Supplier / Manufacturer" icon={<Factory className="w-4 h-4" />}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <Field label="Country of Origin">
                <div className="relative">
                  <select {...register("countryOfOrigin")} className={`${selectCls} pr-9`}>
          ...
          {/* ══ 5. Shipping ══════════════════════════════════════════════ */}
          <SectionCard title="5. Shipping & Trade Terms" icon={<Truck className="w-4 h-4" />}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Field label="Incoterms">
                <div className="relative">
                  <select {...register("incoterms")} className={`${selectCls} pr-9`}>
              ...
              <Field label="Shipping Method">
                <div className="relative">
                  <select {...register("shippingMethod")} className={`${selectCls} pr-9`}>
            ...
            <Field label="Shipping Details / Notes">
              <textarea {...register("shippingNotes")} rows={2}
                placeholder="e.g. Free sea freight for orders above 500 units..."
                className={`${inputCls} resize-none`} />
            </Field>
          </SectionCard>

          {/* ══ 6. Technical Specifications (DYNAMIC) ════════════════════ */}
          <SectionCard title="6. Technical Specifications" icon={<FlaskConical className="w-4 h-4" />}>
            ...
            <div className={`flex items-center gap-2.5 px-4 py-2.5 rounded-xl text-sm font-medium ${
              ...
              {selCat ? (
                <>
                  ...
                  Showing <b className="mx-1">{selCat.name}</b> specifications
                  {!SPEC_BY_SLUG[selCat.slug] && (
                    <span className="text-amber-600 text-xs ml-1">(using default fields)</span>
                  )}
                </>
              ) : (
                <>
                  <AlertCircle className="w-4 h-4 flex-shrink-0 text-amber-500" />
                  Select a category above to load relevant specification fields
                </>
              )}
            </div>
            ...
            <div className="space-y-2">
              {specDefs.map((cat) => {
                const isOpen = openSpec === cat.key;
                const filled = specFilledCount(cat.key);
                const total  = cat.fields.length;
                return (
                  <div key={cat.key} className="rounded-xl border border-gray-200 overflow-hidden">
                    <button type="button"
                      onClick={() => setOpenSpec(isOpen ? null : cat.key)}
                      className="flex w-full items-center justify-between px-4 py-3 bg-white hover:bg-gray-50 transition text-left">
                    ...
                    {isOpen && (
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 px-4 pb-4 pt-3 bg-gray-50 border-t border-gray-100">
                        {cat.fields.map((f) => (
                          <div key={f.key}>
            ...
            <div className="flex items-center justify-between pt-1">
              <p className="text-xs text-gray-400">
                {totalSpecFilled} / {totalSpecFields} spec fields filled
              </p>
              {totalSpecFilled > 0 && (
                <button type="button" onClick={() => setSpecs(buildEmptySpecs(specDefs))}
          ...
          {/* ══ 7. Packaging ════════════════════════════════════════════ */}
          <SectionCard title="7. Packaging & Delivery" icon={<Package className="w-4 h-4" />}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {/* ══ 8. Variations ════════════════════════════════════════════ */}
          <SectionCard title="8. Product Variations" icon={<Tag className="w-4 h-4" />}>
            <div className="space-y-3">
              {variations.map((v, i) => (
                <div key={i} className="grid grid-cols-2 md:grid-cols-4 gap-3 items-center p-3 bg-gray-50 rounded-xl border border-gray-100">
            ...
            <button type="button" onClick={addVariation}
              className="flex items-center gap-1.5 text-sm text-green-600 hover:text-green-700 font-medium mt-1 transition">
              <Plus className="w-4 h-4" /> Add Variation
            </button>
          </SectionCard>

          {/* ══ 9. Tags ══════════════════════════════════════════════════ */}
          <SectionCard title="9. Tags & Keywords" icon={<Tag className="w-4 h-4" />}>
            <div className="flex gap-2">
              <input type="text" value={tagInput}
                onChange={(e) => setTagInput(e.target.value)}
                onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); addTag(); } }}
                placeholder="e.g. gaming, TWS, OEM" className={`${inputCls} flex-grow`} />
              <button type="button" onClick={addTag}
                className="px-4 py-2.5 bg-green-600 text-white rounded-xl hover:bg-green-700 text-sm font-semibold whitespace-nowrap transition">
                Add
              </button>
            </div>
            {tags.length > 0 && (
              <div className="flex flex-wrap gap-2 mt-1">
                {tags.map((t, i) => (
                  <span key={i} className="px-3 py-1 bg-green-100 text-green-700 rounded-full text-sm flex items-center gap-1.5">
                    {t}
                    <X className="w-3.5 h-3.5 cursor-pointer hover:text-green-900"
          ...
          {/* ══ 10. Description ══════════════════════════════════════════ */}
          <SectionCard title="10. Description" icon={<FileText className="w-4 h-4" />}>
          ...
          {/* ══ 11. Media ════════════════════════════════════════════════ */}
          <SectionCard title="11. Product Media" icon={<ImageIcon className="w-4 h-4" />}>
            <Field label="Product Images (Multiple)">
              <label className="flex flex-col items-center justify-center border-2 border-dashed border-gray-200 rounded-xl cursor-pointer p-6 hover:border-green-400 transition bg-gray-50">
            ...
            <Field label="Product Video (Optional)">
              <label className="flex flex-col items-center justify-center border-2 border-dashed border-gray-200 rounded-xl cursor-pointer p-5 hover:border-green-400 transition bg-gray-50">
          ...
          <button type="submit" disabled={submitting}
            className="w-full bg-green-600 text-white py-4 rounded-2xl hover:bg-green-700 active:bg-green-800 transition font-bold text-base shadow-md disabled:opacity-60 flex items-center justify-center gap-2.5">
            {submitting
              ? <><Loader2 className="w-5 h-5 animate-spin" /> Publishing...</>
              : <><Globe2 className="w-5 h-5" /> Publish International Product</>
            }
          </button>
...
//       const res  = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/product`, { method: "POST", body: formData });
//       const json = await res.json();
