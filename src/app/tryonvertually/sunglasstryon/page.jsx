...
const ACCENTS = ["#c9a84c", "#e07b5a", "#5a7be0", "#9b5ae0", "#3dbf7f"];

export default function Sunglasstryon({ product }) {
  ...
  const GLASSES = useMemo(() => {
    ...
    if (productImages.length === 0) {
      return [
        {
          id: 1,
          label: product?.nameEng || product?.name || "Classic Aviator",
          src: img.src,
          emoji: "🕶️",
          accent: "#c9a84c",
        },
    ...
    return productImages.map((src, index) => ({
      ...
      label:
        productImages.length === 1
          ? product?.nameEng || product?.name || "Sunglasses"
          : `${product?.nameEng || product?.name || "Sunglasses"} ${index + 1}`,
      src,
      emoji: "🕶️",
  ...
  useEffect(() => {
    if (overlayRef.current && selectedGlass?.src) {
  ...
  return (
    <>
      <style>{`
        ...
        .gallery-item.selected .gallery-label {
          color: var(--accent);
        }
      ...
      <div className="tryon-root">
        ...
        <div className="tryon-layout" ref={containerRef}>
          <div className="video-wrapper">
            ...
            {mediapipeAvailable && selectedGlass?.src && (
              <Image
                width={300}
                height={120}
                ref={overlayRef}
                ...
                {GLASSES.map((g) => (
                  <div
                    ...
                    className={`gallery-item${
                      selectedGlass?.id === g.id ? " selected" : ""
                    }`}
            ...
            <div className="card">
              ...
              <div className="size-row">
                {["Small", "Medium", "Large"].map((s) => (
                  <div
                    key={s}
                    className={`size-chip${sizeLabel === s ? " active" : ""}`}
                  >
                    {s}
                  </div>
                ))}
...
// // ─── Sunglass Gallery (fallback static data) ────────────────────────────────
...
//   // ─────────────────────────────────────────────
//   // ✅ DYNAMIC GLASSES (API / DB READY)
//   // ─────────────────────────────────────────────
//   const GLASSES = useMemo(() => {
//     // if product images exist → convert to glasses format
//     if (product?.images?.length) {
//       return product.images.map((img, index) => ({
//         id: index + 1,
//         label: product?.nameEng || `Sunglass ${index + 1}`,
...
//     // fallback static
//     return STATIC_GLASSES;
//   }, [product]);

//   const [selectedGlass, setSelectedGlass] = useState(GLASSES[0]);

//   // keep selection synced when product changes
//   useEffect(() => {
//     if (GLASSES?.length) {
//       setSelectedGlass(GLASSES[0]);
//     }
...
//   return (
//     <>
//       {/* (YOUR UI + CSS REMAINS EXACTLY SAME — NO CHANGE) */}
...
//                     style={{ "--accent": g.accent }}
//                   >
//                     <Image width={40} height={40} src={g.src} alt={g.label} />
//                     <span className="gallery-label">{g.label}</span>
//                   </div>
//                 ))}
//               </div>
...
// //                     <Image width={10} height={10} src={g.src} alt={g.label} />
// //                     <span className="gallery-label">{g.label}</span>
// //                   </div>
// //                 ))}
// //               </div>
